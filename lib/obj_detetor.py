...
def get_peason_bbox(image):
    ...
    (rects, weights) = hog.detectMultiScale(image,hitThreshold=0.2,winStride=(4, 4),padding=(16, 16), scale=1.05)
    rects = np.array([[x, y, x + w, y + h] for (x, y, w, h) in rects])
    print (weights)
    pick = non_max_suppression(rects, probs=weights, overlapThresh=0.75)
    print (pick)
    for p in pick:
        if p[0]<1:
            p[0]=1
