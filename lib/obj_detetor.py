...
def get_peason_bbox(image):
    ...
    (rects, weights) = hog.detectMultiScale(image,hitThreshold=0.5, winStride=(2, 2),padding=(24, 24), scale=1.02)


    rects = np.array([[x, y, x + w, y + h] for (x, y, w, h) in rects])
    print (weights)
    pick = non_max_suppression(rects, probs=weights, overlapThresh=0.95)
    for p in pick:
        if p[0]<1:
            p[0]=1
        if p[1]<1:
